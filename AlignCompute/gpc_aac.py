    ...
    def align_and_est(self, n_comp, scale, s, interval_sample=10):
            ...
            if (step_range < 0.01):
                ...
                if (n_comp == 1):
                    ...
                    for n_pca in range(n_comp):
                        G_PCA.add(
                            self.give_me_a_network(E_2[1].loc[n_pca, :], n_a=self.aX.node_attr, e_a=self.aX.edge_attr))
                    self.e_vec = G_PCA
                    self.barycenter_net = self.give_me_a_network(self.barycenter, n_a=self.aX.node_attr,
                                                                 e_a=self.aX.edge_attr)
        ...
        if ('E_2' in locals()):
            ...
            if (n_comp == 1):
                ...
                for n_pca in range(n_comp):
                    G_PCA.add(
                        self.give_me_a_network(E_1[1].loc[n_pca, :], n_a=self.aX.node_attr, e_a=self.aX.edge_attr))
                self.e_vec = G_PCA
                del G_PCA
            del E_1
