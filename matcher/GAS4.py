...
# GAS4 is extremely similar to GAS. It is indeed rewritten in a smarter way.
# For instance, variables are declared all in one push.
...
class GAS4(Matcher):
    ...
    def match(self,X,Y):
        ...
        opt_model.add_constraints_((opt_model.sum(x_vars[i,u] for i in set_I) == 1 for u in set_I),
                                   ("constraint_r{0}".format(u) for u in set_I))

        opt_model.add_constraints_((opt_model.sum(x_vars[i,u] for u in set_I) == 1 for i in set_I),
