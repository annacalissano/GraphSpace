...
class Matcher(object):
    ...
    # dis: take as an imput graph, graph sets or both and compute the sim using the_dis function               
    def dis(self,*args):
        l=len(args)
        if(l>2):
            print 'Hi, consider different input, such as two graphs'
        ...
        if(l==1):
            if(isinstance(args[0],GraphSet)):
                ...
                ########
                # single graph
                print 'No distance for single graph!'
                return 0
                ########
        else:
            if(isinstance(args[0], Graph) and isinstance(args[1], Graph)):
